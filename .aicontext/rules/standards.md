...
- **Interview persistence:** An interview ends when no ambiguities remain or the user explicitly closes — not when they answer the first batch. If an answer opens new branches, keep questioning. Applies to `load-task`, `start-feature`, `interview`, `add-step`, `do-it`, and mid-task discussions.
