...
5. **Test** — if `after_step.tests` resolved to anything other than `false`: compute the changed-files corpus per the Review corpus table above (same rules), call `resolve-tests.md` with the resolved value, `step` context, and the corpus. Pass the returned command(s) to `test-runner` subagent (Claude Code) or run inline (Cursor/Copilot/Codex). If the resolver returns `ERROR`, surface to the user and skip tests.
