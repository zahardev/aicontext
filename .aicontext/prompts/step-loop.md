...
1. **Implement** the step — invalidate readiness and reopen the task before implementation.
...
7. **Close step** — follow `close-step.md`; advance only after its summary is output.
8. **Commit** — if `after_step.commit` is Yes, follow `.aicontext/prompts/commit.md`.
